import { expositorio } from '@prisma/client';
...
export async function procesamientoValidator(req: Request, res: Response, next: NextFunction) {
    ...
    const {expositorio} = req.body; //ojo falta especificar el tipo de expositorio
    ...
    const existingExpositorio: expositorio | null = await expositorioService.getById(expositorio.id_expositorio);
